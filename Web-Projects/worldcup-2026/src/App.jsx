            ...
            <div className="fade-in">
              <div className="cards">
                ...
                <StatCard 
                  ...
                  title={matches.some(m => m.status === "live") ? "🔴 En Vivo Ahora" : "Próximo Partido"} 
                  value={matches.find(m => m.status === "live")?.home_team?.split(" ")[0] + " vs " + matches.find(m => m.status === "live")?.away_team?.split(" ")[0] || "Programado"}
                  sub={matches.some(m => m.status === "live") ? `${matches.find(m => m.status === "live")?.home_score} - ${matches.find(m => m.status === "live")?.away_score}` : "Ver calendario"}
                />
              </div>
